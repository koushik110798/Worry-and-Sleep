...
```
Both the plots and test for normality suggest these residuals are not normal. We will probably want to use a nonparametric method to test the ANOVA hypothesis, a method that doesn’t require the normality assumption.
```
...
```
    Group SA   Group A   Group NA   Group D   Group SD
    ------------------
    --------             ---------
    --------                        --------
    --------                                  ---------
...
##Methode for 2nd research question
...
The test statistic is $X^2 = `r signif(chisq_gg$statistic, 3)`$.

The p-value $= `r signif(chisq_gg$p.value, 3)`$.

Here We have sufficent evidence to to reject our null hypothesis sinceor p-varue is lesser that $\alpha=0.05$. Here we conclude that there is an association between anxity and worry.
...
Ans) Above is pearson residuals and mosaic plot we can see that from the person residual table that there are extreme difference between the values, and this is the reason why we could reject the null hypothesis. if we see at the mosaic plot the people who strongly agree to be worried and say no to anxity are less that those who say yes  this was unexpected and even in the peason residual table we can see  extreme difference in their values. At the same from the mosaic plot we can see three more things the people who say N/A to worry and yes to anxity are more than expected, the peorple who say disagree to worry and yes to anxity are also more than expected and the people who strongly disagree and say yes to anxity are also mosre than expected. We can extreme values there in the pearson rasiduals table.